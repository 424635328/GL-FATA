...
for func_num = 1:length(F)   
    ...
    box_plot = [];  %统计箱型图结果
    %% Run the ASFSSA algorithm for "run" times
    for nrun=1:run
        [final,position,iter]=ASFSSA(pop_size,max_iter,lower_bound,upper_bound,variables_no,fobj);
    ...
    resu = [resu,zz];
    disp(['ASFSSA：最优值:',num2str(zz(1)),' 标准差:',num2str(zz(2)),' 平均值:',num2str(zz(3)),' 中值:',num2str(zz(4)),' 最差值:',num2str(zz(5))]);
    ...
    %% Run the SSA algorithm for "run" times
    for nrun=1:run
    ...
    if box_pp == 1 
        ...
        set(gca,'XTickLabel',{'ASFSSA','DBO','PSO','GWO','SSA'});
...
title = {" ","DBO","PSO","GWO","SSA"};%  秩和检验是和改进的算法做比较
B = [title;B];
