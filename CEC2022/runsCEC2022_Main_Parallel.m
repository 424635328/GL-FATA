...
% ================= 参数设置 =================
force_restart = false; % 设置为 true 以强制清除旧数据，从头开始跑 F1-F12
% ===========================================
    ...
    for a_idx = 1:num_algos
        ...
        if any(need_run_mask)
            parfor nrun = 1:run_times
                ...
                final_fitness = 0; elapsed_time = 0;
                try
                    t_start = tic; 
                    switch alg_name
                        case 'THASO'
                            [~, final_fitness, ~] = THASO(fobj, lb, ub, dim, pop_size, MaxFEs);
                        case 'FATA'
        ...
        if isempty(valid_data)
            ...
            best_v = min(valid_data); std_v = std(valid_data);
            mean_v = mean(valid_data); med_v = median(valid_data);
            worst_v = max(valid_data); avg_t = mean(valid_time);
        end
...
% 1. 统计结果 (Min/Std/Mean/Median/Worst)
try
...
try
    ...
    num_funcs = length(F_list);
    num_comparisons = num_algos - 1;
    RankSum_Symbols = cell(num_funcs, num_comparisons);
    WTL_Count = zeros(3, num_comparisons); % Win, Tie, Loss
    
    for i = 1:num_funcs
        % 提取当前函数下 THASO 的 Mean (位于 RESULT_All 的第 (i-1)*5 + 3 行)
        gl_mean = RESULT_All((i-1)*5 + 3, 1);
        for j = 1:num_comparisons
            comp_idx = j + 1;
            comp_mean = RESULT_All((i-1)*5 + 3, comp_idx);
            pval = RankSum_All(i, j);
            
            if pval < 0.05
                if gl_mean < comp_mean
                    RankSum_Symbols{i,j} = ['+ (', num2str(pval, '%.2e'), ')'];
                    WTL_Count(1, j) = WTL_Count(1, j) + 1;
                else
                    RankSum_Symbols{i,j} = ['- (', num2str(pval, '%.2e'), ')'];
                    WTL_Count(3, j) = WTL_Count(3, j) + 1;
                end
            else
                RankSum_Symbols{i,j} = ['= (', num2str(pval, '%.2e'), ')'];
                WTL_Count(2, j) = WTL_Count(2, j) + 1;
            end
        end
    end
    ...
    disp('已保存: Result_RankSum.xlsx (含 Win/Tie/Loss 统计)');
